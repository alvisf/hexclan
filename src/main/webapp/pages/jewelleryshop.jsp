...
<body>
		...
		<div class="collapse navbar-collapse" id="navbarSupportedContent">
			<ul class="navbar-nav mr-auto">
				...
				<li class="nav-item active">
					<a class="nav-link" href="shopping3">Jewellery Shop <span class="sr-only">(current)</span></a>
				</li>
			</ul>
	...
	<div>
		<h1 class="text-center mt-4">
			<span class="accent-color">JEWELLERY</span> SHOP
		</h1>
	</div>
	<p class="lead text-center mt-2 accent-color">
		Elegant, opulent, radiant.
	</p>
	<p class="text-center caption">Select products to add to your cart.</p>
	<div class="item-list">
		<div class="container">
			<spring:form commandName="items" action="submitShoppingForm3" method="post">
				<div class="row items">
					<c:forEach items="${items}" var="item">
						<div class="col-sm-6 col-md-4 item text-center">
							<input type="checkbox" name="selected" value=<c:out value='${item.getItemNo()}' />> <br />
							<img src=<c:out value='${item.getImgurl()}' /> class="img-fluid" style="width: 200px; height: 200px;">
							<h3 class="name">${item.getItemDesc()}</h3>
							<p class="description">MRP: Rs. ${item.getItemPrice()}</p>
							<p>In Stock: ${item.getStockQuantity()} units</p>
							<select class="form-control mt-2 mb-2 w-50" name=<c:out value='${item.getItemNo()}' />
							style="margin: auto;">
							...
							<div class="col-sm mt-2 mb-4 m-auto">
								<button type="submit"
									class="btn btn-block my-2 my-sm-0 m-auto btn-main text-uppercase">Add to Cart</button>
							</div>
						</div>
					</c:forEach>
				</div>
