				...
				<nav class="navbar navbar-dark bg-dark navbar-expand-lg justify-content-between">
					...
					<div class="container-fluid collapse navbar-collapse" id="navbarNav">
						<ul class="navbar-nav">
							...
							<li class="nav-item"><a class="nav-link" href="/itemcontrol/modifyitem">modifyitem</a></li>
							<li class="nav-item"><a class="nav-link" href="/itemcontrol/modifybill">modifybill</a></li>
						</ul>
					...
					<form method="post">
						<div class="flex">
							...
							<div class="form-group">
								<input name="id" type="text" class="form-control item"
									placeholder="Invoice ID" required="required" />
							</div>

							<div class="form-group">
								<button class="btn btn-block my-2 my-sm-0 m-2 main-button" type="submit" formaction="/itemcontrol/getinvoice">Search by
									Invoice</button>
							</div>
						</div>
					</form>
					...
					<form method="POST">
						...
						<div class="row h-100 justify-content-center align-items-center">
							<div class="col-lg-12 col-md-12 ml-auto mr-auto">
								<p class="text-center">customer name:${custid}</p>
								<p class="text-center">Invoice id:${invid}</p>
								<p class="text-center">Invoice Date:${date}</p>								
															...
															<tbody>
																<c:forEach items="${itemtrancs}" var="itemtranc">
																	<tr>
																		...
																		<td class="text-right">
																		<span>${itemtranc.getItemDetailsDTO().getItemDesc()}</span></td>
        
                                                                        <td><span><input type="checkbox" name="id"
                                                                            checked
                                                                            value=<c:out value='${itemtranc.getTransactionID()}'/>></span></td> 
																		<td class="text-right">
																			<span>${itemtranc.getItemDetailsDTO().getItemCategory()}</span></td>
                                                                        <td class="text-right">
                                                                            <span><select class="form-control" name=<c:out value="${itemtranc.getTransactionID()}" />>
                                                                                <option value=${itemtranc.getItemQuantity()}>${itemtranc.getItemQuantity()}</option>
                                                                                <option value="1">1</option>
                                                                                <option value="2">2</option>
                                                                                <option value="3">3</option>
                                                                                </select></span></td>
																		<td class="text-right">
																			<span>${itemtranc.getItemDetailsDTO().getItemPrice()}</span></td>
                                                                        <td class="text-right">
                                                                            <span><button class="btn btn-block my-2 my-sm-0 m-2 main-button" type="Add" formaction="/itemcontrol/modifybillitem">modify
                                                                                </button></span></td>
                                                                        <td class="text-right">
                                                                            <span><button class="btn btn-block my-2 my-sm-0 m-2 main-button" type="Add" formaction="/itemcontrol/deletebilltem">Delete
                                                                            </button></span></td>        
																	</tr>
																</c:forEach>
															</tbody>
